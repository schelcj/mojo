...
sub build_tx {
  ...
  if (my $p = $self->trusted_proxies) { $tx->req->trusted_proxies($p) }
...
=head2 trusted_proxies
  ...
  $server     = $server->trusted_proxies(['10.0/8', '127.0/8', '172.16.0/12', '192.168.0/16']);

This server expects requests from trusted reverse proxies, defaults to the value of C<MOJO_TRUSTED_PROXIES> environment
variable split on commas with optional whitespace. These proxies should be addresses or networks in CIDR form.
